...
public class SubscribeProductTableDependency : SubscribeTableBase<ProductContract>
{
    SqlTableDependency<Product> _tableProductDependency;

    public SubscribeProductTableDependency(IEventBusInterface<ProductContract> bus, IMapper mapper) : base(bus, mapper)
    {
    }
    ...
    private void TableProduct_Onchange(object sender, RecordChangedEventArgs<Product> e)
    {
        ...
        {
            var contract = _mapper.Map<ProductContract>(e.Entity);
            //contract.SendType = SendType.Delete;
            _bus.PublishMessage(contract);
        }
    }
